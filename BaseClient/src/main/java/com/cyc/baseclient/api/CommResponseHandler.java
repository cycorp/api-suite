...
class CommResponseHandler extends Thread {
   ...
   * @param cycConnection the parent CycConnection
   */
  public CommResponseHandler(Thread parentThread, CycConnection cycConnection, final CycConnection cycComm) {
      ...
      while ((!isClosed) && (!isClosing)) {
        ...
        try {
          ...
          if (cycComm.taskProcessingEnded) {
            if (cycComm.trace > CycConnection.API_TRACE_NONE) {
        ...
        if (cycComm.trace >= CycConnection.API_TRACE_DETAILED) {
        ...
        try {
          if (cycComm.trace >= CycConnection.API_TRACE_MESSAGES) {
          ...
          final CycConnection.WaitingWorkerInfo waitingWorkerInfo = (CycConnection.WaitingWorkerInfo) cycComm.getWaitingReplyThreads().get(id);
