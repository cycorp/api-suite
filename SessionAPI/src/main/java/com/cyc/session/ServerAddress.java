...
public class ServerAddress implements Serializable, Comparable {
  ...
  /**
   * Tests whether a String could be deserialized into a valid ServerAddress. The string 
   * must match the format <code>host:port</code>, the host must be a valid hostname or IP 
   * address, and the port must be a valid port integer. Any string which is validated by this 
   * method should be a well-formed input for {@link #fromString()}.
   * 
 ...
 <code>host:port</code>.
  ...
  /**
   * Checks whether the ServerAddress instance contains enough information to be useful 
  ...
  public int compareTo(Object o) {
    ...
    return new Integer(getPort()).compareTo(addr.getPort());
  }
