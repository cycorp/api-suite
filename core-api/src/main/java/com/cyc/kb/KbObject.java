...
public interface KbObject {
   ...
   */
  public static interface KbObjectWithArity extends KbObject {

    /**
     * Returns the syntactic arity of this object. If it has a getRelation applied to some arguments
     * (i.e. it's a Sentence, an Assertion, or a functional term), the arity is the number of
     * arguments. By convention, Cyc constants have a formula arity of 0.
     *
    ...
    /**
     * Gets the object in <code>argPosition</code> argument position of this KbObject as an object
     * of type <code>O</code>. This method works for Sentences and Assertions, as well as non-atomic
     * KbTerms. However, because a constant has no "arguments", calling this method on a KbObject
     * representing a Cyc constant will result in a KbException.
     *
   ...
   */
  public static KbObject get(String cycLOrId) throws KbTypeException, CreateException {
    return Cyc.getKbObject(cycLOrId);
  }
  
  public static boolean isValidKbApiObjectType(Class cls) {
    return (cls != null)
                   ? VALID_KB_API_CLASSES.stream().anyMatch(c -> c.isAssignableFrom(cls))
                   : false;
  }
  
  public static boolean hasValidKbApiObjectType(Object obj) {
    return (obj != null) ? isValidKbApiObjectType(obj.getClass()) : false;
  }
  
  //====|    Constants    |=======================================================================//
  
  public static final List<Class> VALID_KB_API_CLASSES = Arrays.asList(
          KbObject.class,
          String.class,
          Number.class,
          Date.class);
  ...
  */
   ...
   *
   * <p>
   * Indexicals are those terms which depend upon the occasion of use or the user. For example,
   ...
   *
   * <p>
   * Some indexicals can be resolved automatically by Cyc; for example, Cyc can automatically
   ...
   * with a specific instance of <code>#$InformationStore</code>. This can be accomplished by adding
   * substitutions to a Query; for more information, see
   ...
   *
   * <p>
   * All subclasses of KbObject can be quoted. Refer to <code>#$NoteAboutQuotingInCycL</code> for a
   * more detailed discussion of quoting.
   *
  ...
  */
   ...
   *
   * <p>
   * Refer to <tt>#$NoteAboutQuotingInCycL</tt> for a more detailed discussion of quoting.
   *
   ...
   *
   * <p>
   * See {@link #isQuoted() } for more details about quoted objects.
   *
   ...
   *
   * <p>
   * This method is specifically intended to be called in large batch operations where some
   * KbObjects may be indexicals and others may not, and to be chained with other methods like 
   * {@link KbTerm#replaceTerms(java.util.Map) }, so it attempts to be a little more forgiving than
   * #resolveIndexical(). For this reason, it will not throw an exception simply because the
   ...
   * <li>The object is an indexical which cannot be automatically resolved, and no referent is found
   * in the substitutions map.</li>
   ...
   *
   * <p>
   * For a brief overview of indexicals, see {@link com.cyc.kb.KbObject#isIndexical() }.
   *
  ...
  /**
   * Provides a version of the <code>toString</code> that is suitable for use when talking directly
   * to a Cyc server.
   * <p>
   * The CycL output of each CycObject is required to interact with the underlying KB, for example,
   * to find or create facts.
   *
  ...
  /**
   * Get the CycL representation of <code>this</code> as a String. This representation should be
   * suitable for use with the KbObject factory methods, but may not be suitable for use with
   * lower-level APIs. It should not be expected to contain "#$" constant-prefixes.
   *
