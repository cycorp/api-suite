...
public class RestrictedVariable extends SentenceImpl {
  ...
  */
   ...
   * @param args
   */
  public RestrictedVariable (Context ctx, Variable var, Object... args) throws KbException {
    super(args);
    sentenceArguments = Arrays.asList(args);
    if (!sentenceArguments.contains(var)){
      throw new KbException("The variable " + var + " must be one of the arguments in " + args);
    }
    istContext = ctx;
    variable = var;
