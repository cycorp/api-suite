 ...
 * 
 * @version $Id: CfaslInputStream.java 163356 2016-01-04 20:55:47Z nwinant $
...
public class CfaslInputStream extends BufferedInputStream {
  ...
  public int readInt()
    ...
    if (cfaslOpcode >= CFASL_IMMEDIATE_FIXNUM_OFFSET) {
      ...
      switch (cfaslOpcode) {
        ...
        default:
          reset();
          throw new BaseClientRuntimeException("Expected an int but received OpCode=" + cfaslOpcode);
      }
    }
  }
   ...
   * @throws IOException if a communications error occurs
   */
  public CycSymbolImpl readKeyword()
          throws IOException {
   ...
   * @throws IOException if a communications error occurs
   */
  public GuidImpl readLegacyGuid()
          throws IOException {
    GuidImpl guid = CycObjectFactory.makeGuid((String) readObject());
   ...
   * @throws IOException if a communications error occurs
   */
  public CycArrayList readCycList()
          throws IOException {
    int size = readInt();
    ...
    CycArrayList cycList = new CycArrayList(size);
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (trace == API_TRACE_DETAILED) {
        if (consObject instanceof Fort) {
          debugNote("readCons.consObject: " + ((Fort) consObject).toString());
        } else {
   ...
   * @throws IOException if a communications error occurs
   */
  public CycVariableImpl readCompleteVariable()
          throws IOException {
    final Integer hlVariableId = (Integer) readObject();
    final String name = (String) readObject();
    CycVariableImpl cycVariable = new CycVariableImpl(name, hlVariableId);
    if (trace == API_TRACE_DETAILED) {
  ...
  public CycAssertion readAssertion()
    ...
    if (formulaObject.toString().equals("NIL")) {
      ...
      try {
        formula = (CycArrayList) formulaObject;
