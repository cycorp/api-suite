...
public enum CycServerReleaseType {
   ...
   *
   * This method is more liberal than {@link #valueOf(java.lang.String) } in that it will match on
   * either <em>name</em> or <em>abbreviation</em>, and is case-insensitive. (Like
  ...
  public static CycServerReleaseType fromString(String string) {
    for (final CycServerReleaseType type : values()) {
      if (!OTHER.equals(type)) {
        if (type.getName().equalsIgnoreCase(string) || type.getAbbreviation().equalsIgnoreCase(string)) {
          return type;
        }
      }
    }
