...
public interface QueryResultSet extends Iterable<QueryAnswer> {
   ...
   */
  public <T> T getObject(Variable columnVar, Class<T> type);
   ...
   * primitive, with an expected type of <code>T</code>. Note that you may get a ClassCastException
   * later, because this may not actually give you back a T.
   *
   * @param <T> the expected type of the value.
   * @param columnVar the variable for which a binding is sought.
   * @param type the expected type of the value.
   ...
   * @throws ArrayIndexOutOfBoundsException
   */
  public <T> T getKbObject(int columnIndex, Class<T> type) 
          throws IllegalArgumentException, KbException;
   ...
   * method that takes an integer for maximum performance.
   *
   * @param columnVar the name of the variable that represents the column of
   * interest
   ...
   * row at the column identified by <code>columnIndex</code> position. Returns
   * <code>null</code> if no value is set for the current row and given column.
   *
  ...
  /**
   * Corresponds to {@link java.sql.ResultSet#first()}. Moves the cursor to
   * the first row.
   *
  ...
  /**
   * Corresponds to {@link java.sql.ResultSet#last()}. Moves the cursor to
   * the last row.
   *
   * @return <code>true</code> if the cursor is on a valid row;
   * <code>false</code> if there are no rows in the result set
  ...
  /**
   * Corresponds to {@link java.sql.ResultSet#absolute(int)}. Moves the
  ...
  /**
   * Corresponds to {@link java.sql.ResultSet#relative(int)}. Moves the
  ...
  /**
   * Corresponds to {@link java.sql.ResultSet#previous()}. Moves the cursor
   * to the previous row.
   *
