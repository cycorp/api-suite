 ...
 * @author nwinant
 */
public class KbTermFactory {
  
  // Static
  
  private static final KbTermFactory ME = new KbTermFactory();

  protected static KbTermFactory getInstance() {
    return ME;
  }
  ...
  /**
   * Find or create a <code>KbTerm</code> object named <code>nameOrId</code>, and also make it an
   ...
   * <code>nameOrId</code>, one will be created, and it will be asserted to be an instance of
   * <code>constraintCol</code>. If there is already an object in the KB called
   * <code>nameOrId</code>, and it is already a <code>constraintCol</code>, it will be returned. If
   * it is not already a <code>constraintCol</code>, but can be made so by addition of assertions to
   * the KB, such assertions will be made, and the object will be returned. If the object in the KB
   * cannot be turned into a <code>constraintCol</code> by adding assertions, a
   ...
   * of <code>constraintCol</code>. If there is already an object in the KB called
   * <code>nameOrId</code>, and it is already a <code>constraintCol</code>, it will be returned. If
   * it is not already a <code>constraintCol</code>, but can be made so by addition of assertions to
   * the KB, such assertions will be made, and the object will be returned. If the object in the KB
   * cannot be turned into a <code>constraintCol</code> by adding assertions, a
  ...
  /**
   * Checks whether entity exists in KB and is an instance of #$Thing. If false,
   ...
   * an instance of <code>#$Thing</code>.
   *
   * @param nameOrId either the name or HL ID of an entity in the KB
